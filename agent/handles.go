...
type HandleCache struct {
	mu        sync.RWMutex
	Cache     map[uint32]map[string]map[uintptr]*HandleEntry // pid -> object type -> handle
...
func (c *HandleCache) Init() {
	...
	defer c.mu.Unlock()
	if c.Cache == nil {
		c.Cache = make(map[uint32]map[string]map[uintptr]*HandleEntry)
	}
	for _, handle := range handleTable {
		if c.Cache[handle.Pid] == nil {
			c.Cache[handle.Pid] = make(map[string]map[uintptr]*HandleEntry)
		}
		objectType := handle.GetTypeName()
		if c.Cache[handle.Pid][objectType] == nil {
			c.Cache[handle.Pid][objectType] = make(map[uintptr]*HandleEntry)
		}
