...
//  while remote memory allocation events are a subgroup. Theses groups define which condition sets can be used.
...
//TODO: write on paper ALL possible groups/events.
//TODO: write on paper which condition sets are possible per each

//TODO: which group does an event belong to
//TODO: which condition sets are allowed for this group
//TODO: which set a specific condition belongs to

func (event FileComponent) GetGroup() string {
	return GROUP_FILE_EVENT
}

func (event RegComponent) GetGroup() string {
	return GROUP_REG_EVENT
}

func (event ApiComponent) GetGroup() string {
...
func GetApiGroup(options []string) string {
	...
	for _, option := range options {
		...
		//* actual api
		switch option {
		case "VirtualAlloc": //TODO: which other options?
			groups[GROUP_LOCAL_MEM_ALLOC] = true
		case "VirtualAllocEx": //TODO: which other options?
			groups[GROUP_REMOTE_MEM_ALLOC] = true
		case "VirtualProtect": //TODO: which other options?
			groups[GROUP_LOCAL_MEM_PROTECT] = true
		case "VirtualProtectEx": //TODO: which other options?
		...
		case "GetProcAddress":
			groups[GROUP_GET_FN_ADDRESS]
		case "LoadLibraryA", "LoadLibraryW", "LoadLibraryExA", "LoadLibraryExW": //TODO: other options like rtl, nt
			groups[GROUP_LOAD_LIBRARY]
...
func GetConditionSets(group string) []Condition {
	var sets []Condition
	switch group {
...
func GetConditionSetNames() []string {
	//TODO: is this needed?
}
